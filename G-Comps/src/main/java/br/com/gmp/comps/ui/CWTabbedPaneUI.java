...
/**
 * An implementation of the TabbedPaneUI that looks like the tabs that were used
 * in the older versions of the Codewarrior IDE.
 * <p/>
...
public class CWTabbedPaneUI extends BasicTabbedPaneUI {
    ...
    private static final Insets NO_INSETS = new Insets(1, 1, 1, 1);
     ...
     */
    private Color selectedTextColor = BaseColors.alternativeColor;
     ...
     */
    public CWTabbedPaneUI(Color selectedTextColor) {
    ...
    protected void installComponents() {
        ...
        tabColor = BaseColors.darkColor;//tabPane.getBackground().darker().darker();
    ...
    protected int calculateTabHeight(int tabPlacement, int tabIndex, int fontHeight) {
        if (tabPlacement == tabIndex) {
            ...
            return buttonHeight + (buttonHeight / 2) + 3;
        }
    }
    ...
    protected void paintTabArea(Graphics g, int tabPlacement, int selectedIndex) {
        ...
        int x = tabPlacement;//rects[rects.length - 1].x + rects[rects.length - 1].width;
...
//            g.setColor(Color.green);//fadeColors[fadeColorCount - 1]);
...
//        }
    ...
    protected void paintTabBorder(Graphics g, int tabPlacement, int tabIndex, int tx, int ty, int tw, int th, boolean isSelected) {
        ...
        if (isSelected) {
            ...
            g.drawLine(0, buttonHeight, buttonHeight / 2, buttonHeight
                    + (buttonHeight / 2));

            g.setColor(backgroundDarker2);
            g.drawLine(0, buttonHeight + 1, buttonHeight / 2, buttonHeight
                    + (buttonHeight / 2) + 1);
            ...
            g.setColor(Color.black);
            g.drawLine(buttonHeight / 2 + 1, buttonHeight + (buttonHeight / 2)
                    + 2, tw - buttonHeight / 2 - 1, buttonHeight
                    + (buttonHeight / 2) + 2);
            g.drawLine(tw - buttonHeight / 2 - 1, buttonHeight
                    + (buttonHeight / 2) + 2, tw, buttonHeight + 1);
            // Removida a sombra
//            for (int i = 1; i <= fadeColorCount; i++) {
    ...
    protected void paintText(Graphics g, int tabPlacement, Font font,
        ...
        if (isSelected) {
            ...
            g.drawString(title, (r.width / 2 - fm.stringWidth(title) / 2) + 1,
                    buttonHeight / 2 + fm.getMaxDescent()
                    + buttonHeight / 2 + 3);
        } else {
            FontMetrics fm = getFontMetrics();
